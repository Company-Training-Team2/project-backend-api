...
public class WorkPostService : IWorkPostService
{
    ...
    private static readonly Expression<Func<WorkPost, WorkPostSummaryDto>> ToSummaryDto = w => new WorkPostSummaryDto
    {
        ...
        // Filter-then-project in one hop (not .Select(nav).Where(...).Select(...)) —
        // that chain made EF Core's query translator throw
        // "Expression of type IQueryable<Double> cannot be used for parameter of
